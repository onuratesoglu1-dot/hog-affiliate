...
export default async function PanelPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: clicks } = await supabase
    ...
    .select("user_id, paid, paid_at, clicked_at, installed")
    .eq("slug", slug);
  ...
  // Bu ay aktif ödeme (aylık ödeme hesabı)
  const now = new Date();
  const firstOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  const active_this_month = clicks?.filter(c => c.paid).length || 0;
  const commission_per_user = 50; // TL — bunu değiştirebilirsin
  const expected_payment = active_this_month * commission_per_user;

  const card = (label: string, value: string | number, color: string) => `
    <div style="background:#151515;border:1px solid #262626;border-radius:16px;padding:20px;text-align:center;flex:1;min-width:140px">
  ...
  const rows = clicks?.slice(0, 50).map((c, i) => `
    <tr style="border-bottom:1px solid #1a1a1a">
      <td style="padding:10px;color:#888;font-size:12px">${new Date(c.clicked_at).toLocaleDateString("tr-TR")}</td>
      <td style="padding:10px;color:#888;font-size:12px">${c.user_id ? "✓ İndirdi" : "—"}</td>
    ...
    <html>
      ...
      <body style={{ margin: 0, backgroundColor: "#0B0B0B", color: "#fff", fontFamily: "sans-serif", padding: "24px", minHeight: "100vh" }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <div style={{ background: "#151515", border: "1px solid #FF7A00", borderRadius: 16, padding: 20, marginBottom: 24, textAlign: "center" }}>
            <div style={{ color: "#888", fontSize: 13, marginBottom: 6 }}>Bu Ay Beklenen Ödeme</div>
            <div style={{ fontSize: 36, fontWeight: 900, color: "#FF7A00" }}>{expected_payment.toLocaleString("tr-TR")} ₺</div>
            <div style={{ color: "#555", fontSize: 12, marginTop: 4 }}>{active_this_month} aktif kullanıcı × {commission_per_user} ₺</div>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #1a1a1a" }}>
                  <th style={{ padding: "8px 10px", color: "#555", fontSize: 11, textAlign: "left", fontWeight: 600 }}>Tarih</th>
                  <th style={{ padding: "8px 10px", color: "#555", fontSize: 11, textAlign: "left", fontWeight: 600 }}>İndirme</th>
                  <th style={{ padding: "8px 10px", color: "#555", fontSize: 11, textAlign: "left", fontWeight: 600 }}>Ödeme</th>
                </tr>
              </thead>
              <tbody dangerouslySetInnerHTML={{ __html: rows || `<tr><td colspan="3" style="padding:20px;text-align:center;color:#555">Henüz veri yok</td></tr>` }} />
            </table>
          </div>

          <div style={{ textAlign: "center", marginTop: 24, color: "#333", fontSize: 12 }}>
            Her ayın 1'inde aktif kullanıcı sayısına göre ödeme yapılır.
          </div>
        </div>
      </body>
    </html>
